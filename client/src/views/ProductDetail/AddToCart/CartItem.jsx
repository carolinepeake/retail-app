...
import { CloseButton, SquareCloseButton } from '../../../components/Buttons';
import QuantityDropdown from './QuantityDropdown';

// make function returning price jsx

function CartItem({
  item,
  setCart,
}) {
  // TODO: deletes all items in cart with the deleted sku - this is good b/c all items of one sku will make 1 cart item
  // if want to reduce quantity of sku in bag, change quantity dropdown in bag - need to make sure this reduces quantity

  const removeProductFromCart = (e) => {
    const skuToDelete = e.target.id;
    setCart((prev) => (prev.slice().filter((cartItem) => (Number(cartItem.sku) !== Number(skuToDelete)))));
  };
  ...
  // TODO: make shared getPrice function or component
...
const TextContainer = styled.div`
  ...
  padding: 0.5em;
  position: relative;
  font-size: ${(props) => props.theme.tertiary}; /* 0.83 ? */
  border-radius: 7.5px;
 /* background-color: ${(props) => props.theme.backgroundColor}; */
`;
...
const Name = styled.h4`
  ...
  font-weight: 300;
  font-weight: 500;
  /* dark blue or purple or that brown grey ? */
`;
...
const SelectedQuantity = styled.select`
  ...
  /* TODO: make something happen on hover */
...
//   background-color: transparent; /* not sure I like this */
...
/*  border: 1px lightgrey solid; */
  background-color: rgba(129,52,224,0.1)
`;

// const Delete = styled(SquareCloseButton)`
//   height: 1.2em;
//   font-size: 1em;
//   top: 0;
//   right: 0;
